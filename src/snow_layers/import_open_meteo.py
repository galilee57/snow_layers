...
from snow_layers.repository import MERIBEL, get_or_create_meribel, upsert_observations
...
def main() -> None:
    ...
    records, source_url = fetch_daily_snow_depth(
        latitude=MERIBEL["latitude"],
        longitude=MERIBEL["longitude"],
        elevation_m=MERIBEL["elevation_m"],
    ...
    print(f"{count} jours importés pour Méribel ({start_date} → {end_date}).")
